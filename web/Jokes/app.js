async function Joke(type,cat){
    ...
    // console.log(`https://v2.jokeapi.dev/joke/${cat}?type=${type}`)
    return res;
    }
    catch(er){
        return -1
...
function multiSelection(name){
    let arr = document.getElementById(name);
    const selectedValues = [].filter
    .call(arr.options, option => option.selected)
    .map(option => option.text);
    let out = selectedValues.join(",")
    return out
}
...
document.querySelector(".submit").addEventListener("click",async () =>{
    ...
    let cat = multiSelection("category");
