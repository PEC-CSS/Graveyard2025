...
filter_type = input("Choose filter type (Actor, Director, or Genre (Case, Spaces, Spelling Sensitive)): ").strip().capitalize()
filter_value = input(f"Enter {filter_type} name: ").strip().lower()


if filter_type == "Actor":
    filtered_df = df[df['Stars'].str.contains(filter_value, case=False, na=False)]
elif filter_type == "Director":
    filtered_df = df[df['Director'].str.contains(filter_value, case=False, na=False)]
elif filter_type == "Genre":
    filtered_df = df[df['Genre'].apply(lambda genres: any(filter_value in g for g in genres))]
else:
    print("Invalid filter type!")
    exit()


M = int(input("Enter the number of top movies to display (M): "))
M = min(M, len(filtered_df))
